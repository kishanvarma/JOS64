...
send_data(struct http_request *req, int fd)
{
	...
	//get the stats of the corresponding fd to get the size of file
	int d,err;
        struct Stat stat;
        if( (d = fstat(fd,&stat)) < 0){
//                if( (err = send_error(req,404)) < 0 )
                        cprintf("\nsend error:getting fstat failed\n");
        }
	...
	//read from the fd for the file size.
	if( (len_read = read(fd, data, n)) < 0)
		die("\n failure in read \n");

	//now write the data on the socket.
	ssize_t len_sent = write(req->sock, data, len_read);

	if( len_sent != len_read)
		 die("Failed to send bytes to client");
	return 0;
...
send_file(struct http_request *req)
{
	...
	//using fstat for checking if file is a directory
	int d;
	struct Stat stat;
	if( (d = fstat(fd,&stat)) < 0){
		if( (err = send_error(req,404)) < 0 )
	...
	if(stat.st_isdir == 1){
		if( (err = send_error(req,404)) < 0 )
                        cprintf("\nsend error: file is a directory\n");
        }
