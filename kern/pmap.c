...
pml4e_t *boot_pml4;							// Kernel's initial page directory
...
static void boot_map_region(pml4e_t *pml4, uintptr_t va, size_t size, physaddr_t pa, int perm);
...
boot_alloc(uint32_t n)
{
	...
	//
 	...
 	if (n > 0) {
	//	uint32_t allocated_size = ROUNDUP(n,PGSIZE);
 		//resukt now points to next free
		result = nextfree;        
		//update  nextfree pointer.    
		nextfree = ROUNDUP((char *)(nextfree + n),PGSIZE);
		//I beleive upper limit of Kernel virtual address is large enough for the scope here.not panicking		  	
		//size: 20, address: 0x0000000000100000, length: 0x000000000fee0000, type: 1
		if(nextfree >(char*) KADDR(0xfee0001)) {
		panic("boot alloc : out of memory exception  ");	
		}	 
 }
	else 
	    result = nextfree ;
...
//	cprintf("\n npages : %d \n",npages);
	//since for each physical page there is corresponding struct ,we multiply the structs size with npages.
	//////////////////////////////////////////////////////////////////////
...
//adding the check after pageinit and before bootmap
...
//check_page_free_list(0);  
	...
	// LAB 3: Your code here.
	boot_map_region(boot_pml4,UENVS,ROUNDUP(NENV * sizeof(struct Env),PGSIZE),PADDR(envs),PTE_U|PTE_P);
//	cprintf("\n NENV : %d \n",NENV);
	boot_map_region(boot_pml4,UPAGES,ROUNDUP(npages*sizeof(struct PageInfo),PGSIZE),PADDR(pages),PTE_U|PTE_P);
//>>>>>>> lab2
	//////////////////////////////////////////////////////////////////////
	...
	// Your code goes here:
	  //here: la=KSTACKTOP-KSTACKSIZE(stack grows down), pa=PADDR(bootstack),size=KSTKSIZE,perm = PTE_U | PTE_P
	boot_map_region(boot_pml4,KSTACKTOP-KSTKSIZE,KSTKSIZE,PADDR(bootstack),PTE_P|PTE_W);
	// boot_map_region(boot_pml4,KSTACKTOP-PTSIZE,PTSIZE-KSTKSIZE,PADDR(bootstack),PTE_P|PTE_W);
	//////////////////////////////////////////////////////////////////////
...
//	mem_init_mp();
	...
	boot_map_region(boot_pml4,KERNBASE,npages * PGSIZE,(physaddr_t)0x0,PTE_P|PTE_W);
...
mem_init_mp(void)
{
	...
	boot_map_region(boot_pml4,kstacktop_i - KSTKSIZE ,KSTKSIZE ,PADDR(percpu_kstacks[i]),PTE_P|PTE_W);
	}
}
...
/**             //IO hole must never be allocated,and memory for intial boot table must never be allocated, that means even this must be marked as used 
                if(page2pa(&pages[i]) >= PADDR(KERNBASE + IOPHYSMEM) && page2pa(&pages[i]) <= PADDR(KERNBASE + EXTPHYSMEM) 
                        && page2pa(&pages[i])>=PADDR(BOOT_PAGE_TABLE_START) && page2pa(&pages[i]) <= PADDR(BOOT_PAGE_TABLE_END)){
                        pages[i].pp_link = NULL ;
                        pages[i].pp_ref = 1;
                        }
**/
//from the #define in memorylayout.h
//From the kernel, IOPHYSMEM can be addressed at KERNBASE + IOPHYSMEM. 
//included the lower bound after discussion . not sure earlier.
...
page_alloc(int alloc_flags)
{
	...
	//p1 points to head of page_free_list,then we update page_free_list and allocate mem to p1 node using memset.
	struct PageInfo *p1 = page_free_list;
	if(p1 == NULL){
		return NULL;
	}
	if(p1->pp_link == p1)   //last one
	    page_free_list = NULL;
	else
...
page_free(struct PageInfo *pp)
{
	...
	if(pp != NULL){
		...
		panic("The PAGE is being USED or already free");
		
		//updating the page_free_list.
...
// Given a pml4 pointer, pml4e_wcalk returns a pointer
...
// If the allocation fails , pml4e_walk returns NULL. Otherwise,
// the new page's reference count is incremented, the page is cleared,
// and it calls pdpe_walk() with the given relevant pdpe_t pointer
...
pml4e_walk(pml4e_t *pml4, const void *va, int create)
{
    // LAB 2: Fill this function in
    if(pml4 == NULL ) {
        panic("the pml4e pointer error");
    }
        size_t offset = PML4(va);
        pml4e_t pml4_index = pml4[offset];
        pdpe_t *pdpe = NULL;
        pte_t *pte = NULL;
        if (pml4_index & PTE_P ) {
            pdpe = (pdpe_t*)KADDR(PTE_ADDR(pml4_index));
            ...
            if(create) {
                ...
                pml4[offset] = (pml4e_t)pdpe;
                pml4[offset] |= PTE_P | PTE_W | PTE_U ;
		pdpe_t*  pdpe_virtual =(pdpe_t*) KADDR(PTE_ADDR(pml4[offset]));
		memset(pdpe_virtual,0,PGSIZE);
                pte = pdpe_walk(pdpe_virtual,va,create);
		if(pte == NULL) {
			page_decref(pp);
			pml4[offset] = 0;
			pp->pp_link=NULL;
...
pdpe_walk(pdpe_t *pdp, const void *va, int create){
    size_t offset = PDPE(va);
    pdpe_t pdpe_index = pdp[offset];
    pde_t *pgdir = NULL;
    pte_t *pte = NULL;
    if(pdpe_index & PTE_P) {
        pgdir = (pde_t*)KADDR(PTE_ADDR(pdpe_index));
        ...
        if(create) {
            ...
            pdp[offset] = (pdpe_t)pgdir;
            pdp[offset] |= PTE_P | PTE_W | PTE_U ;
            pde_t*  pgdir_virtual =(pde_t*) KADDR(PTE_ADDR(pdp[offset]));
	    memset(pgdir_virtual,0,PGSIZE);
            pte = pgdir_walk(pgdir_virtual,va,create);
...
{
    size_t offset = PDX(va);
    pde_t pgdir_index = pgdir[offset];
    pte_t *pte = NULL;
    if(pgdir_index & PTE_P) {
        pte = (pte_t*)KADDR(PTE_ADDR(pgdir_index));
        offset = PTX(va);
        pte = &pte[offset];
    }
        ...
        if(create) {
            ...
            if(pp == NULL)
                return NULL;
            pp->pp_ref+=1;
            pgdir[offset] = page2pa(pp);
            pgdir[offset]|= PTE_P | PTE_W |PTE_U ;
	    memset(page2kva(pp),0,PGSIZE);
            pte = (pte_t*)KADDR(PTE_ADDR(pgdir[offset]));
            offset = PTX(va);
            pte = &pte[offset];
...
static void
boot_map_region(pml4e_t *pml4, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	// LAB 2: Fill this function in
	size_t i;
	for(i=0;i<size;i=i+PGSIZE)
	{
	pte_t *ptentry = pml4e_walk(pml4,(void *)(la + i),1);
	if(ptentry != NULL){
		*ptentry = PTE_ADDR(pa + i ); 
		*ptentry |= perm |PTE_P;
	}
	}
}
...
{	
	...
	pte_t *ptentry = pml4e_walk(pml4e,va,1);
	
	if(ptentry == NULL){
        	//cprintf("\nreturning negative value\n");
   		return -E_NO_MEM;
	}
	//if a page is there ,the corner should be checked before the next condition.
	if(PTE_ADDR(*ptentry) == page2pa(pp)){
	//	cprintf("\nsame page thing\n");
		*ptentry = PTE_ADDR(*ptentry) | perm | PTE_P;
		 tlb_invalidate(pml4e,va);
		return 0;
		}	
	//if already a page is there, then remove it.
	if(*ptentry != 0){
 		page_remove(pml4e,va);
		tlb_invalidate(pml4e,va);
	}	

	//increment pp_ref and store in the ptentry index the pa of the page.
	*ptentry = page2pa(pp);
	*ptentry =  *ptentry | perm | PTE_P;
	(pp->pp_ref)++;
	return 0;
}
...
{
	// Fill this function i
//	cprintf("\n inside page lookup. \n ");
	pte_t *ptentry = pml4e_walk(pml4e,va,0);
	//create = 0,we are not creating pages here :D 
	if(ptentry == NULL)
		return NULL;
	if(pte_store != NULL)
		*pte_store = ptentry;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	// LAB 4: Your code here.
	size_t round_size = ROUNDUP(size,PGSIZE);	//round 'size' up .. rechecking,,  

//	cprintf("\n MMIOBase: %x \t base : %x \t size : %x \t MMIOLIM : %x \n",MMIOBASE,base,size,MMIOLIM);	
	if(base + round_size > MMIOLIM ){
		panic("\nover flow in mmio_map_region");
	}
	uintptr_t result = base;
	
	boot_map_region(boot_pml4,base,round_size,pa,PTE_PCD|PTE_PWT|PTE_W|PTE_P);
...
{
	// LAB 3: Your code here.
//	cprintf("\nstarting user mem check\n");
	struct PageInfo *pp = NULL;
	
	uint64_t va_start = ROUNDDOWN((uint64_t)va,PGSIZE);
	uint64_t va_len_end = ROUNDUP((uint64_t)va+len,PGSIZE);
	uint64_t i;
	
	//perm |= PTE_P;
	pte_t *pte = NULL;

	for(i=va_start;i < va_len_end;i+=PGSIZE) {
		pp = page_lookup(env->env_pml4e,(void *)i,&pte);
		
		if( (pp == NULL) || (i >= ULIM) || ((*pte & perm)!= perm )){
		 //    cprintf("\n pp %d ,i  %d,  perm  %d  \n", (pp==NULL) ,(i > ULIM) ,((*pte & perm)!= perm ));
                     user_mem_check_addr = (uintptr_t)va;
		     if(i > user_mem_check_addr)
...
//>>>>>>> lab3
	...
	pdpe_t *pdpe = KADDR(PTE_ADDR(boot_pml4[1]));
...
page_check(void)
{
	...
	//cprintf("\n success just before stealing the free pages\n");
	// temporarily steal the rest of the free pages
	fl = page_free_list;
	page_free_list = NULL;
...
//	cprintf("\nafter\n");
	...
	assert(check_va2pa(boot_pml4, 0) == page2pa(pp1));
	assert(check_va2pa(boot_pml4, PGSIZE) == page2pa(pp1));
	...
	page_remove(boot_pml4, 0x0);
	assert(check_va2pa(boot_pml4, 0x0) == ~0);
	assert(check_va2pa(boot_pml4, PGSIZE) == page2pa(pp1));
	assert(pp1->pp_ref == 1);
	assert(pp3->pp_ref == 1);
	...
	page_remove(boot_pml4, (void*) PGSIZE);
	assert(check_va2pa(boot_pml4, 0x0) == ~0);
	assert(check_va2pa(boot_pml4, PGSIZE) == ~0);
	assert(pp1->pp_ref == 0);
	assert(pp3->pp_ref == 1);
	...
	page_decref(pp2);
	va = (void*)(PGSIZE * 100);
	ptep = pml4e_walk(boot_pml4, va, 1);
	...
	pml4e_walk(boot_pml4, 0x0, 1);
	pdpe = KADDR(PTE_ADDR(boot_pml4[0]));
	...
	assert(check_va2pa(boot_pml4, mm1) == 0);
	assert(check_va2pa(boot_pml4, mm1+PGSIZE) == PGSIZE);
	assert(check_va2pa(boot_pml4, mm2) == 0);
	assert(check_va2pa(boot_pml4, mm2+PGSIZE) == ~0);
	// check permissions
	assert(*pml4e_walk(boot_pml4, (void*) mm1, 0) & (PTE_W|PTE_PWT|PTE_PCD));
	assert(!(*pml4e_walk(boot_pml4, (void*) mm1, 0) & PTE_U));
	// clear the mappings
	*pml4e_walk(boot_pml4, (void*) mm1, 0) = 0;
	*pml4e_walk(boot_pml4, (void*) mm1 + PGSIZE, 0) = 0;
	*pml4e_walk(boot_pml4, (void*) mm2, 0) = 0;
