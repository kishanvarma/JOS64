...
env_setup_vm(struct Env *e)
{
	...
	// LAB 3: Your code here.
	(p->pp_ref)++;
	physaddr_t phys_env = page2pa(p);
	pml4e_t *kva_env = page2kva(p);
	size_t UTOP_offset = PML4(UTOP);        //offset of UTOP address in the pml4 page table.
//	cprintf("\n UTOP_offset : %d \n ", UTOP_offset);
	e->env_pml4e = kva_env;  
	e->env_cr3 = phys_env;
	        	
	for(i = UTOP_offset ; i < NPMLENTRIES ; i++)
	       e->env_pml4e[i] = boot_pml4[i];
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	size_t i;
	size_t  va_down = ROUNDDOWN((uint64_t)va,PGSIZE);
	size_t  va_len_up = ROUNDUP((uint64_t)va+len,PGSIZE);

	for(i = va_down ; i <va_len_up ; i+= PGSIZE)
	{
		struct PageInfo *pp = page_alloc(ALLOC_ZERO);
		if(pp == NULL)
			panic("\npage not allocated\n");
		(pp->pp_ref)++;	
		if(page_insert(e->env_pml4e , pp ,(void *) i , PTE_U | PTE_W | PTE_P) < 0 )
			panic("page could not be inserted\n");
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	//
	//  All page protection bits should be user read/write for now.                  PTE_U|PTE_W.
	//  ELF segments are not necessarily page-aligned, but you can
	//  assume for this function that no two segments will touch
	//  the same virtual page.
	//
	//  You may find a function like region_alloc useful.                            to allocate the regions.
	//
	//  Loading the segments is much simpler if you can move data
	//  directly into the virtual addresses stored in the ELF binary.
	//  So which page directory should be in force during                     loading this environments page directory.
	//  this function?
	//
	...
	ph = (struct Proghdr *) ((uint8_t *) ELFHDR + ELFHDR->e_phoff);  //from here
        eph = ph + ELFHDR->e_phnum;                                      //to here
	...
	//how to set this as entry point?
...
//	e->env_tf.tf_rsp = USTACKTOP ;
	...
	// LAB 3: Your code here
        // Now map one page for the program's initial stack
        // at virtual address USTACKTOP - PGSIZE.
	region_alloc(e,(void*)USTACKTOP - PGSIZE , PGSIZE) ;  //e,vasize of one page.	
...
//=======
	struct Env *e;
	if(env_alloc(&e,0) < 0)
		panic("\nenvironment couldnt be allocated in env_create\n"); 

	load_icode(e,binary);
	e->env_type = type;
	
	if(type == ENV_TYPE_FS){
 		e->env_tf.tf_eflags |=	FL_IOPL_MASK; //setting thr flag, mmu.h
	}	
//>>>>>>> lab4
}
...
env_free(struct Env *e)
{
	...
	// If freeing the current environment, switch to the kernel page table
	// before freeing the environments page table, just in case the pages
	// get reused.
	if (e == curenv)
		lcr3(boot_cr3);
...
env_run(struct Env *e)
{
	...
	if((curenv != NULL) && (curenv->env_status == ENV_RUNNING)) 
	  ...
	  lcr3(curenv->env_cr3);
	  unlock_kernel();
	  env_pop_tf(&e->env_tf);//hint and step2.
	
//	cprintf("\nenv_run is implemented\n");
