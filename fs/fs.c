...
//
// Returns:
//  Block number allocated on success, or
...
alloc_block(void)
{
	...
	for( i=1 ;i < super->s_nblocks ;i++){
	 	 if(block_is_free(i)){
			//getting idea from free_block to write snippet for 'use'
        		bitmap[i/32] &= ~(1<<(i%32)) ;	
...
// Hint: Don't forget to zero-out any block you allocate.
int
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	int total_blocks = f->f_size/BLKSIZE ;
	if(filebno > total_blocks)
		return -E_NOT_FOUND;
	...
	if(f->f_indirect == 0){
		...
		memset((uint32_t*)diskaddr(f->f_indirect),0,BLKSIZE);
	  	...
	  	uint32_t* indirect_array = (uint32_t*)(diskaddr(f->f_indirect));
		*ppdiskbno = &indirect_array[filebno - NDIRECT]; //because NDIRECT blocks do not come in the indirect array.
		return 0;
